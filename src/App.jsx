...
export function Hyperlink({text, href, id}){
    return <a id={id} className="list" href={href}>{text}</a>
}
export function Input({text, type, id, value}){
    return <>
        <label className="label">{text}</label>
        <input type={type} id={id} defaultValue={value}></input>
    </>
}
export function Button({text, id, className, onClick}){
    return (
...
export function Div({id, className, content, style, onClick}){
    return <div className={className + " div"} id={id} style={style} onClick={onClick}>
        {content}
    </div>
}
export function Audio({id, name = "Lovely Piano Song"}){
    const audioElement = useRef();

    return (<>
        <audio ref={audioElement} controls id={id} style={{"position": "absolute", "left": "10px", "bottom": "10px", "zIndex": "100"}}>
            {/* <source src={"assets/" + name + ".mp3"}></source> */}
